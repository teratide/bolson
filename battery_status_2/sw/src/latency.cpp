...
static auto input_schema_1() -> std::shared_ptr<arrow::Schema>
{
...
static auto input_schema_2() -> std::shared_ptr<arrow::Schema>
{
  static auto result = fletcher::WithMetaRequired(
      *arrow::schema({arrow::field("input",
                                   arrow::uint8(),
...
static auto output_schema_1() -> std::shared_ptr<arrow::Schema>
{
  static auto result = fletcher::WithMetaRequired(
...
static auto output_schema_2() -> std::shared_ptr<arrow::Schema>
{
  static auto result = fletcher::WithMetaRequired(
      *arrow::schema({arrow::field("voltage", output_type(), false)}),
      "output_2",
...
int main(int argc, char **argv)
{
  ...
  uint8_t *offset_data_1 = huge_page();
  memset(offset_data_1, 0, HUGE_PAGE_SIZE);
  auto offset_buffer_1 = std::make_shared<arrow::Buffer>(offset_data_1, HUGE_PAGE_SIZE);
  uint8_t *value_data_1 = huge_page();
  // uint8_t *value_data = (uint8_t *)memalign(2 * 1024 * 1024, buffer_size);
  auto value_buffer_1 = std::make_shared<arrow::Buffer>(value_data_1, HUGE_PAGE_SIZE);
  auto value_array_1 = std::make_shared<arrow::PrimitiveArray>(arrow::uint64(), 0, value_buffer_1);
  auto list_array_1 = std::make_shared<arrow::ListArray>(arrow::list(arrow::uint64()), 0, offset_buffer_1, value_array_1);
  std::vector<std::shared_ptr<arrow::Array>> arrays_1 = {list_array_1};
  auto output_batch = arrow::RecordBatch::Make(output_schema(), 0, arrays_1);

  uint8_t *offset_data_2 = huge_page();
  memset(offset_data_2, 0, HUGE_PAGE_SIZE);
  auto offset_buffer_2 = std::make_shared<arrow::Buffer>(offset_data_2, HUGE_PAGE_SIZE);
  uint8_t *value_data_2 = huge_page();
  // uint8_t *value_data = (uint8_t *)memalign(2 * 1024 * 1024, buffer_size);
  auto value_buffer_2 = std::make_shared<arrow::Buffer>(value_data_2, HUGE_PAGE_SIZE);
  ...
  auto output_batch = arrow::RecordBatch::Make(output_schema(), 0, arrays_2);
  ...
  auto tiny_record_batch_1 = arrow::RecordBatch::Make(input_schema_1(), TINY_RECORD.size(), {tiny_record_array});
  auto tiny_record_batch_2 = arrow::RecordBatch::Make(input_schema_2(), TINY_RECORD.size(), {tiny_record_array});
  ...
  context->QueueRecordBatch(tiny_record_batch_1);
  context->QueueRecordBatch(tiny_record_batch_2);
  context->QueueRecordBatch(output_batch_1);
  context->QueueRecordBatch(output_batch_2);
  context->Enable();
  std::cerr << std::endl;
  ...
  std::cerr << 2 * TINY_RECORD.size() / tiny_record.time() << " bytes per second" << std::endl;
  ...
  auto small_record_batch = arrow::RecordBatch::Make(input_schema_1(), SMALL_RECORD.size(), {small_record_array});
  auto small_record_batch = arrow::RecordBatch::Make(input_schema_2(), SMALL_RECORD.size(), {small_record_array});

  fletcher::Platform::Make(PLATFORM, &platform, false);
  platform->init_data = &guid;
  platform->Init();
  fletcher::Context::Make(&context, platform);
  context->QueueRecordBatch(small_record_batch_1);
  context->QueueRecordBatch(small_record_batch_2);
  context->QueueRecordBatch(output_batch_1);
  context->QueueRecordBatch(output_batch_2);
  context->Enable();
  std::cerr << std::endl;
  ...
  std::cerr << 2 * SMALL_RECORD.size() / small_record.time() << " bytes per second" << std::endl;
  platform->Terminate();
  ...
  auto large_record_batch = arrow::RecordBatch::Make(input_schema_1(), LARGE_RECORD.size(), {large_record_array});
  auto large_record_batch = arrow::RecordBatch::Make(input_schema_2(), LARGE_RECORD.size(), {large_record_array});

  fletcher::Platform::Make(PLATFORM, &platform, false);
  platform->init_data = &guid;
  platform->Init();
  fletcher::Context::Make(&context, platform);
  context->QueueRecordBatch(large_record_batch_1);
  context->QueueRecordBatch(large_record_batch_2);
  context->QueueRecordBatch(output_batch_1);
  context->QueueRecordBatch(output_batch_2);
  context->Enable();
  std::cerr << std::endl;
  ...
  std::cerr << 2 * LARGE_RECORD.size() / large_record.time() << " bytes per second" << std::endl;
  platform->Terminate();
  ...
  auto multiple_tiny_record_batch_1 = arrow::RecordBatch::Make(input_schema_1(), MULTIPLE * TINY_RECORD.size(), {multiple_tiny_record_array});
  auto multiple_tiny_record_batch_2 = arrow::RecordBatch::Make(input_schema_2(), MULTIPLE * TINY_RECORD.size(), {multiple_tiny_record_array});

  fletcher::Platform::Make(PLATFORM, &platform, false);
  platform->init_data = &guid;
  platform->Init();
  fletcher::Context::Make(&context, platform);
  context->QueueRecordBatch(multiple_tiny_record_batch_1);
  context->QueueRecordBatch(multiple_tiny_record_batch_2);
  context->QueueRecordBatch(output_batch_1);
  context->QueueRecordBatch(output_batch_2);
  context->Enable();
  std::cerr << std::endl;
  ...
  std::cerr << MULTIPLE * 2 * TINY_RECORD.size() / multiple_tiny_record.time() << " bytes per second" << std::endl;
  platform->Terminate();
  ...
  auto multiple_small_record_batch_1 = arrow::RecordBatch::Make(input_schema_1(), MULTIPLE * SMALL_RECORD.size(), {multiple_small_record_array});
  auto multiple_small_record_batch_2 = arrow::RecordBatch::Make(input_schema_2(), MULTIPLE * SMALL_RECORD.size(), {multiple_small_record_array});

  fletcher::Platform::Make(PLATFORM, &platform, false);
  platform->init_data = &guid;
  platform->Init();
  fletcher::Context::Make(&context, platform);
  context->QueueRecordBatch(multiple_small_record_batch_1);
  context->QueueRecordBatch(multiple_small_record_batch_2);
  context->QueueRecordBatch(output_batch_1);
  context->QueueRecordBatch(output_batch_2);
  context->Enable();
  std::cerr << std::endl;
  ...
  std::cerr << MULTIPLE * 2 * SMALL_RECORD.size() / multiple_small_record.time() << " bytes per second" << std::endl;
  platform->Terminate();
  ...
  auto multiple_large_record_batch_1 = arrow::RecordBatch::Make(input_schema_1(), MULTIPLE * LARGE_RECORD.size(), {multiple_large_record_array});
  auto multiple_large_record_batch_2 = arrow::RecordBatch::Make(input_schema_2(), MULTIPLE * LARGE_RECORD.size(), {multiple_large_record_array});

  fletcher::Platform::Make(PLATFORM, &platform, false);
  platform->init_data = &guid;
  platform->Init();
  fletcher::Context::Make(&context, platform);
  context->QueueRecordBatch(multiple_large_record_batch_1);
  context->QueueRecordBatch(multiple_large_record_batch_2);
  context->QueueRecordBatch(output_batch_1);
  context->QueueRecordBatch(output_batch_2);
  context->Enable();
  std::cerr << std::endl;
  ...
  std::cerr << MULTIPLE * 2 * LARGE_RECORD.size() / multiple_large_record.time() << " bytes per second" << std::endl;
  platform->Terminate();
}