...
struct StreamThreads {
  ...
  void Shutdown(convert::Converter *converter) {
    shutdown.store(true);
    converter->Stop();
    publish_thread->join();
  }
...
auto ProduceFromStream(const StreamOptions &opt) -> Status {
  ...
  if (std::holds_alternative<illex::RawProtocol>(opt.protocol)) {
    ...
    switch (opt.converter.implementation) {
      ...
      case parse::Impl::OPAE_BATTERY: {
        return Status(Error::GenericError, "OPAE battery status not enabled in this build.");
      }
...
//      }
    ...
    // TODO: wrap into a function from up here
    ...
    auto conv_stats = converter.stats;
